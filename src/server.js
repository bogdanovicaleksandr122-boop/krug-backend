...
// crossOriginResourcePolicy переключаем на "cross-origin": по умолчанию helmet
// ставит "same-origin", а это именно то, что не давало фото специалистов
// (запрос идёт с github.io на другой домен railway.app) грузиться в <img> —
// браузер молча блокировал ответ как cross-origin, поэтому фото было видно
// только в самом Telegram-боте, но не на карточке в приложении.
